...
class Basis1D:
    ...
    def fourier_transform_array(self, midpoints, J, wave_numbers):
        ...
        self.ell_tilde_inv = 1.0 / ell_tilde
        
        plt.figure()
        for i in range(8):
            plt.plot(wave_numbers, np.real(ell_tilde[i, :]), 'o--', label='real: node'+str(i))
        plt.legend(loc='best'), plt.grid(True)
        plt.figure()
        for i in range(8):
            plt.plot(wave_numbers, np.imag(ell_tilde[i, :]), 'o--', label='imag: node'+str(i))
        plt.legend(loc='best'), plt.grid(True)
        plt.figure()
        for i in range(8):
            plt.plot(wave_numbers, np.absolute(ell_tilde[i, :]), 'o--', label='magn: node'+str(i))
        plt.legend(loc='best'), plt.grid(True)
        
        plt.figure()
        for i in range(8):
            plt.plot(wave_numbers, np.real(self.ell_tilde_inv[i, :]), 'o--', label='real: node'+str(i))
        plt.legend(loc='best'), plt.grid(True)
        plt.figure()
        for i in range(8):
            plt.plot(wave_numbers, np.imag(self.ell_tilde_inv[i, :]), 'o--', label='imag: node'+str(i))
        plt.legend(loc='best'), plt.grid(True)
        plt.figure()
        for i in range(8):
            plt.plot(wave_numbers, np.absolute(self.ell_tilde_inv[i, :]), 'o--', label='magn: node'+str(i))
        plt.legend(loc='best'), plt.grid(True)
        ...
        self.inv_phases = 1.0 / phase
        self.inv_array = np.transpose(np.multiply(self.inv_phases[:, :, None], self.ell_tilde_inv.T), (0, 2, 1))
